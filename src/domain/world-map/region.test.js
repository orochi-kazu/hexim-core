...
describe('region', () => {
  ...
  const buildBoard = () => buildSharedBoard(2, mapSize.xs, { wrapping: false }).tiles
  const prepTileForAdd = ({ tiles, x, y, type, player = thisPlayer }) => {
  ...
  const initWithTile = ({ x, y, type }) => {
    ...
    const startingTile = prepTileForAdd({ tiles, x, y, type })
    const r = region(startingTile)

    return { tiles, startingTile, r }
  }
  ...
  describe('add', () => {
    ...
    it("aborts if the tile's type doesn't match the region's", () => {
      const { tiles, r } = initWithTile({ x: 4, y: 5, type })
      const addTile = prepTileForAdd({ tiles, x: 3, y: 5, type: tileTypes.wilderness })
    ...
    it("aborts if the tile's player doesn't match the region's", () => {
      const { tiles, r } = initWithTile({ x: 5, y: 4, type })
      const addTile = prepTileForAdd({ tiles, x: 6, y: 4, type, player: anotherPlayer })
    ...
    it("aborts if the tile doesn't touch the region", () => {
      const { tiles, r } = initWithTile({ x: 2, y: 2, type })
      const addTile = prepTileForAdd({ tiles, x: 6, y: 6, type })
    ...
    it('adds the tile when possible, updates turn count', () => {
      const { tiles, r } = initWithTile({ x: 4, y: 4, type })
      const addTile = prepTileForAdd({ tiles, x: 3, y: 4, type })
  ...
  describe('tiles', () => {
    ...
    it('keeps track of tiles as the region grows', () => {
      const { tiles, r } = initWithTile({ x: 1, y: 1, type })
      ...
      additions.forEach(({ x, y }, i) => {
        const addTile = prepTileForAdd({ tiles, x, y, type })
        const added = r.add(addTile)
  ...
  describe('find free neighbors', () => {
    ...
    it('finds regular free spaces', () => {
      ...
      const x3 = 3
      const y = 3
      const { tiles, startingTile, r } = initWithTile({ x: x2, y, type })
      ...
      expect(nList.includes(tiles[x1][y])).toEqual(true)
      expect(nList.includes(tiles[x3][y])).toEqual(true)

      let addTile = prepTileForAdd({ tiles, x: x1, y, type })
      r.add(addTile)
      ...
      addTile = prepTileForAdd({ tiles, x: x3, y, type })
      r.add(addTile)
      n = r.findFreeNeighbors()
      nList = setToList(n)
    ...
    it('clips free spaces by map edges', () => {
      ...
      const y2 = 1
      const y3 = 2
      const { tiles, startingTile, r } = initWithTile({ x, y: y2, type })
      ...
      expect(nList.sort()).toEqual(startingTile.neighbours.toSorted())
      expect(nList.includes(tiles[x][y1])).toEqual(true)
      expect(nList.includes(tiles[x][y3])).toEqual(true)

      let addTile = prepTileForAdd({ tiles, x, y: y1, type })
      r.add(addTile)
      n = r.findFreeNeighbors()
      nList = setToList(n)
      ...
      addTile = prepTileForAdd({ tiles, x, y: y3, type })
      r.add(addTile)
      n = r.findFreeNeighbors()
      nList = setToList(n)
    ...
    it('clips free spaces by types', () => {
      ...
      const x3 = 6
      const y = 7
      const { tiles, r } = initWithTile({ x: x2, y, type })

      prepTileForAdd({ tiles, x: x2, y: y - 1, type: otherType })
      prepTileForAdd({ tiles, x: x2, y: y + 1, type: otherType })
