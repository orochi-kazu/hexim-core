...
export const hexBoard = (polarity, width, height, { wrapping, tileExtrasBuilder = () => ({}) }) => {
  if (wrapping) {
  ...
  const neighbourlessTiles = buildNeighbourlessTiles(width, height, tileExtrasBuilder)
  const tiles = addNeighbours(neighbourlessTiles, polarity, { wrapping })

  const tilesMatching = lambda => tiles.flatMap(col => col.filter(lambda))
...
const buildNeighbourlessTiles = (width, height, tileExtrasBuilder) =>
  Array(width).keys().map(x =>
    Array(height).keys().map(y =>
      tile(tileExtrasBuilder({ x, y }), { x, y })
    ).toArray()
  ).toArray()

const addNeighbours = (tiles, polarity, { wrapping }) => {
  ...
  const maybeNeighbour = (x, y) =>
    [x, y].includes(undefined) ? undefined : tiles[x][y]

  const dirs = neighbourDirections[polarity]
  tiles.forEach((col, x) => col.forEach((t, y) => {
