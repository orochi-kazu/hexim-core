...
export const regionRepo = (player, board) => {
  ...
  const mergeRegions = list => {
    const dest = list.shift()
    while (list.length > 0) {
      const src = list.shift()
      const added = dest.subsumeRegion(src)
      if (!added) {
        logAndThrow('Failed to merge region', { dest, src })
      }
      const rIndex = regions.indexOf(src)
      regions.splice(rIndex, 1)
    }
  }

  return Object.freeze({
    __debugReadOnly: { board, regions, freeNeighboursFor, regionsNeighouring, canClaimSpot, mergeRegions },
    player,
    allRegions,
    regionsByType,
    claimableSpots,
